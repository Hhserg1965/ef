...
extern char node_id[100];
extern unsigned long long  node_cnt;
extern int node_idn;
//extern int node_idn;
extern char jstype_flg;
...
HHMapIDF_T del_map;// = new HHMapIDF_T();
ef_Fac ef;
ef_Content ec;

//HHMapIDF_T *del_map = NULL;//new HHMapIDF_T();

hVal *vT = new hVal(hVal::_TRUE, 0,0);
hVal *vF = new hVal(hVal::_FALSE, 0,0);
...
hVal *vNL = new hVal(hVal::CSTRING, new std::string("\015\012"),0);
...
char *tps[] ={
    "T","F","N", "R","B","RP","CDOUBLE","CSTRING", "CMAP","CDEQ",
    "DOUBLE","STRING","MAP","DEQ","FNC","FNCC","BUF",
    "LOCK","DB","SQL","UDP","RE"
};
...
//        printf("REAL_delete %lf %ld %d %d %s\n",v->v.d,v->v.vl,v->b.c,v->b.tp,tp_f(v->b.tp).c_str());fflush(stdout);
        ...
        if( v->v.vl){
            ...
            case hVal::STRING:
            case hVal::BUF:
//                printf("delete %lf %ld c: %d ntp: %d tp: %s\n",v->v.d,v->v.vl,v->b.c,v->b.tp,tp_f(v->b.tp).c_str());
//                printf("%s\n",v->v.s->c_str());
...
//                ml.unlock();
            ...
            {
                RE_T * d = (RE_T *)v->v.vl;
                if( d->md) pcre2_match_data_free(d->md);   /* Release memory used for the match */
                if(d->re) pcre2_code_free(d->re);                 /* data and the compiled pattern. */
                delete d;
            }
                break;
...
void ef_Content::dI(int id,DEL_FNC v){
//    dm->insert( std::pair<int, DEL_FNC>(id, v));
    del_map.insert( std::pair<int, DEL_FNC>(id, v));
}
void ef_Content::fI(std::string k,hVal*v){
    f->insert( std::pair<std::string, hVal*>(k,v));
}
void ef_Content::sP(hVal*v){
    sd->push_back(v);
}
void ef_Content::sPB(){
    sd->pop_back();
}
void ef_Content::sB(hVal*v){
    sd->back() = v;
}
hVal* ef_Content::gN(){
    return vN;
...
void ef_Content::ts(hVal *v,std::string &o){
    lg_hVal(*this,v,0,o,0);
}

/*
void ef_Content::gI(int id,hVal*v){


}
void ef_Content::sI(int id,hVal*v){
...
//    std::cerr << "<--LOGGG +++\n"; std::cerr.flush();
    ...
    std::string o;
    mtx_lg.lock();
    std::cerr << lg_hVal(ec,ec.sd->back(),0,o,0)<<'\n'; std::cerr.flush();
    mtx_lg.unlock();
...
static hVal* re_f(ef_Content &ec)
{
    hVal *r = vN;
    if( ec.sd->size()<= 0) return 0;
    ...
    re = pcre2_compile(
      (PCRE2_SPTR)v->v.s->c_str(),               /* the pattern */
...
static hVal* split_f(ef_Content &ec)
{
    hVal *r = vN;
    ...
    if( v->b.tp != hVal::STRING && v->b.tp != hVal::CSTRING && v->b.tp != hVal::BUF) return 0;
    if( d->b.tp != hVal::STRING && d->b.tp != hVal::CSTRING && d->b.tp != hVal::BUF) return 0;

    r = ec.ef.new_hVal(hVal::DEQ,0,0);

    char *dl = (char *)v->v.s->c_str();
    int dlsz = v->v.s->size();
    char *p = (char *)d->v.s->c_str();
    char *pn = 0;

    while(1){
        pn = strstr(p,dl);
        if( pn){
            r->push(ec.ef.new_hVal(hVal::STRING,ec.ef.new_str(p,pn-p),0));
            p = pn + dlsz;
        }else{
            break;
        }
    ...
    r->push(ec.ef.new_hVal(hVal::STRING,ec.ef.new_str(p),0));
...
static hVal* join_f(ef_Content &ec)
{
    hVal *r = vN;
    ...
    if( v->b.tp != hVal::STRING && v->b.tp != hVal::CSTRING && v->b.tp != hVal::BUF) return 0;
    if( d->b.tp != hVal::DEQ && d->b.tp != hVal::CDEQ) return 0;
    ...
    while( qi != q->end()){
        register hVal * tv = *qi;
        if( tv->b.tp == hVal::STRING || tv->b.tp == hVal::CSTRING || tv->b.tp == hVal::BUF){
            if( c == 0) s += *tv->v.s;
            else s += *v->v.s + *tv->v.s;
    ...
    ec.sd->back() = ec.ef.new_hVal(hVal::STRING,ec.ef.new_str(s),0);
...
static hVal* lgs(ef_Content &ec)
{
    ...
    mtx_lg.lock();
    std::cerr << "\n#-->LOG_STACK " << id << std::to_string((long long)ec.sd->size()) << " (size)\n"; std::cerr.flush();
...
*/
    std::string ps="123 456 789";
    int r = ps.find("123");

    std::cout << ps << " FIND " << r << std::endl;
...
oops:
      ...
      if( d->b.tp == hVal::STRING || d->b.tp == hVal::CSTRING || d->b.tp == hVal::BUF){
          RE_T *re = (RE_T *)z->v.vl;

          uint32_t options = 0;                   /* Normally no options */
          PCRE2_SIZE start_offset = 0;   /* Start at end of previous match */
          int flg_end = false;

          char *s = (char*)d->v.s->c_str();
          if(s == re->s){
            PCRE2_SIZE *ovector = pcre2_get_ovector_pointer(re->md);
            start_offset = ovector[1];
            if (ovector[0] == ovector[1]){
              if (ovector[0] == d->v.s->size()) flg_end = true;
              re->rc = -1;
          ...
          if( !flg_end){
            re->rc = rc = pcre2_match(
            re->re,                   /* the compiled pattern */
            (PCRE2_UCHAR*)s,              /* the subject string */
            d->v.s->size(),       /* the length of the subject */
            start_offset,                    /* start at offset 0 in the subject */
            options,                    /* default options */
            re->md,           /* block for storing the result */
...
//          ec.sd->pop_back();
    ...
    if( z->b.tp != hVal::_TRUE && (zz->b.tp != hVal::MAP && zz->b.tp != hVal::DEQ && zz->b.tp != hVal::CMAP && zz->b.tp != hVal::CDEQ)){
        ec.sd->pop_back();
        delete z;
        ++st->back();
        return 0;
...
op:
    ...
    if( z->b.tp == hVal::_FALSE || z->b.tp == hVal::NIL){
        flg_rev = 1;
        ec.sd->pop_back();
        if( ec.sd->size() == 0) goto op;
        z = ec.sd->back();
    }
//    ec.sd->pop_back();
...
//        delete v;
    ...
    if( z->b.tp == hVal::DEQ || z->b.tp == hVal::CDEQ){
        ...
        int i = 0;
        hVal *e = ec.ef.new_hVal(hVal::DOUBLE,0,(double)0);
        ec.sd->push_back(e);
...
/*
static hVal* uuid_f(ef_Content &ec){

    ec.sd->push_back(ec.ef.new_hVal(hVal::STRING,newUUID(),0));
...
void getTIDN(char *s)
{
    ...
    sprintf(s,"%08X%02x%06X",t,node_idn % 256,node_cnt % 16777216);
}
*/
...
static hVal* gtid_f(ef_Content &ec){
    char b[100];

    getTID(b);

    ec.sd->push_back(ec.ef.new_hVal(hVal::STRING,ec.ef.new_str(b),0));

    return 0;
}

static unsigned long tmi_cnt = 0;
static hVal* gtmi_f(ef_Content &ec){
    char b[100];
    ++tmi_cnt;

    time_t t = time(0);

    sprintf(b,"%08X%06X",t,tmi_cnt % 16777216);
...
static hVal* vnn_f(ef_Content &ec,int n1, int n2){
    int v_sz = ec.sdv->size()-n1;

    if( v_sz < 0) return 0;

    int cur = ec.sdv->at(v_sz)-n2;
    if( cur < 0) return 0;
...
static hVal* v30_f(ef_Content &ec){ return (vnn_f(ec,4,1));}
static hVal* v31_f(ef_Content &ec){ return (vnn_f(ec,4,2));}
static hVal* v32_f(ef_Content &ec){ return (vnn_f(ec,4,3));}
static hVal* v33_f(ef_Content &ec){ return (vnn_f(ec,4,4));}
static hVal* v34_f(ef_Content &ec){ return (vnn_f(ec,4,5));}
static hVal* v35_f(ef_Content &ec){ return (vnn_f(ec,4,6));}
static hVal* v36_f(ef_Content &ec){ return (vnn_f(ec,4,7));}
static hVal* v37_f(ef_Content &ec){ return (vnn_f(ec,4,8));}
static hVal* v38_f(ef_Content &ec){ return (vnn_f(ec,4,9));}
static hVal* v39_f(ef_Content &ec){ return (vnn_f(ec,4,10));}

static hVal* v40_f(ef_Content &ec){ return (vnn_f(ec,5,1));}
static hVal* v41_f(ef_Content &ec){ return (vnn_f(ec,5,2));}
static hVal* v42_f(ef_Content &ec){ return (vnn_f(ec,5,3));}
static hVal* v43_f(ef_Content &ec){ return (vnn_f(ec,5,4));}
static hVal* v44_f(ef_Content &ec){ return (vnn_f(ec,5,5));}
static hVal* v45_f(ef_Content &ec){ return (vnn_f(ec,5,6));}
static hVal* v46_f(ef_Content &ec){ return (vnn_f(ec,5,7));}
static hVal* v47_f(ef_Content &ec){ return (vnn_f(ec,5,8));}
static hVal* v48_f(ef_Content &ec){ return (vnn_f(ec,5,9));}
static hVal* v49_f(ef_Content &ec){ return (vnn_f(ec,5,10));}

static hVal* v50_f(ef_Content &ec){ return (vnn_f(ec,6,1));}
static hVal* v51_f(ef_Content &ec){ return (vnn_f(ec,6,2));}
static hVal* v52_f(ef_Content &ec){ return (vnn_f(ec,6,3));}
static hVal* v53_f(ef_Content &ec){ return (vnn_f(ec,6,4));}
static hVal* v54_f(ef_Content &ec){ return (vnn_f(ec,6,5));}
static hVal* v55_f(ef_Content &ec){ return (vnn_f(ec,6,6));}
static hVal* v56_f(ef_Content &ec){ return (vnn_f(ec,6,7));}
static hVal* v57_f(ef_Content &ec){ return (vnn_f(ec,6,8));}
static hVal* v58_f(ef_Content &ec){ return (vnn_f(ec,6,9));}
static hVal* v59_f(ef_Content &ec){ return (vnn_f(ec,6,10));}
...
*/
    ...
    if( (v->b.tp == hVal::STRING || v->b.tp == hVal::CSTRING) && (b->b.tp == hVal::STRING  || b->b.tp == hVal::CSTRING)) {
        v->set(ec.ef.new_str(*b->v.s));
//        v->set(b->v.s);
//        b->v.vl = NULL;
...
//        v->set(b->v.m);
        ...
        register HHMapSV_T *o = b->v.m;
        register HHMapSV_TI it = o->begin();
        while( it != o->end()){
            hVal *z = it->second;
            ++z->b.c;

            std::pair<HHMapSV_TI,bool> ret = m->insert ( std::pair<std::string, hVal*>(it->first,z) );
...
//    fprintf(stderr,"... 0 \n"); fflush(stderr);
    ...
    hVal *tv = new hVal(v->b.tp,v->v.vl,0);
    delete tv;
...
static hVal* sqI_f(ef_Content &ec){

    register hVal *b = ec.sd->back();
    if( (b->b.tp == hVal::CDOUBLE  || b->b.tp == hVal::DOUBLE)){
        int n = b->v.d;
        delete b;
        ec.sd->pop_back();
        qset_f(ec,n);
...
static hVal* swapPlus_f(ef_Content &ec){
    swap_f(ec);
    plus_f(ec);
    return 0;
}
...
//        ec.sd->push_back(ec.ef.new_hVal(hVal::DOUBLE,0,v->v.s->length()));
        r = ec.ef.new_hVal(hVal::DOUBLE,0,v->v.s->length());
...
//        ec.sd->push_back(ec.ef.new_hVal(hVal::DOUBLE,0,v->v.m->size()));
        r = ec.ef.new_hVal(hVal::DOUBLE,0,v->v.m->size());
...
//        ec.sd->push_back(ec.ef.new_hVal(hVal::DOUBLE,0,v->v.q->size()));
        r = ec.ef.new_hVal(hVal::DOUBLE,0,v->v.q->size());
...
/*
static hVal* utP_f(ef_Content &ec){
    int dp = ec.sd->size() - 2;
    ...
    if( (v->b.tp == hVal::DOUBLE || v->b.tp == hVal::CDOUBLE) && (d->b.tp == hVal::STRING || d->b.tp == hVal::CSTRING)){
        std::string ss = "";
        double dd = v->v.d;
...
//        time_t tm = time
        ...
        hVal * r = ec.ef.new_hVal(hVal::STRING,ec.ef.new_str(ss),0);
...
//        ec.sd->push_back(r);
    ...
    if( (v->b.tp == hVal::DOUBLE || v->b.tp == hVal::CDOUBLE) && (d->b.tp == hVal::STRING || d->b.tp == hVal::CSTRING)){
        ...
        if( ddd != 0){
            ss += std::to_string((long double)dd);
        }else{
            ss += std::to_string((long long)dd);
        }
...
//        hVal * r = ec.ef.new_hVal(hVal::DOUBLE,0,d->v.d + v->v.d);
    ...
    if( (v->b.tp == hVal::STRING || v->b.tp == hVal::CSTRING) && (d->b.tp == hVal::MAP || d->b.tp == hVal::CMAP)){
        ...
        if( !ret.second) {
            ...
            it->second = ec.ef.new_hVal(hVal::DOUBLE,0,2);//???? COUNTER!!!!!
//            it->second = vT;//ec.ef.new_hVal(hVal::DOUBLE,0,2);//????
...
rr:
    ...
    if( (d->b.tp == hVal::STRING || d->b.tp == hVal::CSTRING)){

        hVal * r = ec.ef.new_hVal(hVal::STRING,ec.ef.new_str(*d->v.s + tp_f(v->b.tp)),0);
        delete v;
        delete d;
...
static hVal* zv_f(ef_Content &ec){
    ...
    if( (v->b.tp == hVal::DOUBLE || v->b.tp == hVal::CDOUBLE) && (d->b.tp == hVal::DEQ || d->b.tp == hVal::CDEQ)){
        hVal * r = vN;
        int pos = v->v.d;
        if( v->v.d < 0 ){
            pos = d->v.q->size() + v->v.d;
        }
        delete v;
...
static hVal* and_f(ef_Content &ec){
    ...
    if( (v->b.tp == hVal::_TRUE ) && (d->b.tp == hVal::_TRUE )){
        ret = vT;
        goto end;
    }
...
//    hVal *d = ec.sd->at(dp);
    ...
    if( b->b.tp <= hVal::NIL){
        ec.sd->back() = (ec.ef.new_hVal(hVal::DOUBLE,0,(double)time(0)));
        return 0;
    }

    if( (b->b.tp == hVal::DOUBLE || b->b.tp == hVal::CDOUBLE)){
        hVal * r = vN;
        time_t dd = b->v.d;
        struct tm * tl = localtime (&dd);

        char buf[100];
        sprintf(buf,"%04d-%02d-%02d %02d:%02d:%02d",tl->tm_year+ 1900,tl->tm_mon+1,tl->tm_mday,tl->tm_hour,tl->tm_min,tl->tm_sec);

//        fprintf(stderr,"b: %s\n",b);fflush(stderr);

        r = ec.ef.new_hVal(hVal::STRING,ec.ef.new_str(buf),0);
...
//        fprintf(stderr,"b4: %s\n",b);fflush(stderr);
    ...
    if( b->b.tp == hVal::STRING || b->b.tp == hVal::CSTRING){
        ...
        char *p = (char*)b->v.s->c_str();
        tl.tm_year = atol(p) - 1900;
        int c = 0;
        ...
        time_t nt = mktime( &tl);
        r = ec.ef.new_hVal(hVal::DOUBLE,0,(double)nt);
...
{
    void *v = (void*)uv_thread_self();
    long l = (long)v;
    double d = (double)l;
    ec.sd->push_back(ec.ef.new_hVal(hVal::DOUBLE,0,d));
...
static hVal* rows_f(ef_Content &ec)
{
    ...
    ec.sd->push_back(ec.ef.new_hVal(hVal::DOUBLE,0,atol(PQcmdTuples(p->res)) ));
    return 0;
}
...
std::string *toUL_f(char *s,int sl,int flg)
{
    ...
    if( flg){
        r = utf8ucstr(s, sl, &szDst, &iDstLen);
    }else{
        r = utf8lcstr(s, sl, &szDst, &iDstLen);
    }

    if( r){
        return 0;
    }else{
        std::string *rs = new std::string(szDst);
//        free(szDst);
        return rs;
    }
...
static hVal* toU_f(ef_Content &ec)
{

    if( ec.sd->size() == 0){
        std::cerr << "ERROR: 'toU_f' zero size stack!\n"; std::cerr.flush();
        return 0 ;
    }
    ...
    std::string * s = toUL_f((char*)v->v.s->c_str(),v->v.s->size(),1);

    if( s){
        ec.sd->push_back(ec.ef.new_hVal(hVal::STRING,s,0));
    }else{
        ec.sd->push_back(vN);
    }
...
static hVal* toL_f(ef_Content &ec)
{
    ...
    std::string * s = toUL_f((char*)v->v.s->c_str(),v->v.s->size(),0);
...
static hVal* SH2UL_f(ef_Content &ec)
{

    if( ec.sd->size() == 0){
        std::cerr << "ERROR: 'SH2UL_f' zero size stack!\n"; std::cerr.flush();
        return 0 ;
    }
    ...
    char s[1000], *end;
    unsigned long z = strtoul( v->v.s->c_str(), &end, 16);
    sprintf(s,"%lu",z);

    delete v;

    ec.sB(ec.ef.new_hVal(hVal::STRING,ec.ef.new_str(s),0));

    return 0;
}

static hVal* urlE_f(ef_Content &ec)
{
    fprintf(stderr,"urlE_f  0\n");fflush(stderr);

    if( ec.sd->size() == 0){
        std::cerr << "ERROR: 'urlE_f' zero size stack!\n"; std::cerr.flush();
        return 0 ;
    }
...
static hVal* urlD_f(ef_Content &ec)
{
fprintf(stderr,"urlD_f  0\n");fflush(stderr);
...
static hVal* reQ_f(ef_Content &ec)
{

    if( ec.sd->size() == 0){
        std::cerr << "ERROR: 'reQ_f' zero size stack!\n"; std::cerr.flush();
        return 0 ;
    }
    ...
    hVal *r = ec.ef.new_hVal(hVal::DEQ,0,0);
    for(int i = 0; i < re->rc; i++){
      PCRE2_SPTR substring_start = (PCRE2_SPTR)re->s + ovector[2*i];
      size_t substring_length = ovector[2*i+1] - ovector[2*i];
//      printf("%2d: %.*s\n", i, (int)substring_length, (char *)substring_start);
      r->push(ec.ef.new_hVal(hVal::STRING,ec.ef.new_str((char*)substring_start,substring_length),0));
    }
...
//                return handle_scope.Close(Boolean::New(false));
        ...
        char *pp = 0;
        if( !bpn && (pp = strstr(blk,"\015\012\015\012"))) {
            *pp++ = 0;
            *pp++ = 0;
            *pp++ = 0;
            *pp++ = 0;
...
//            fprintf(stderr,"**** 4 %d\n",pp);fflush(stderr);
        ...
        }else if( !bpn && (pp = strstr(blk,"\012\012"))) {
            *pp++ = 0;
            *pp++ = 0;
            bp = pp;
            bpn = pp-blk;//-pp;
...
/*
    if( !del_map) {
        del_map = new HHMapIDF_T();//HHMapIDF_T
        ec.dm = del_map;
    }
*/
    ec.wl();
...
//    ec.wu();
    ...
    ec.f->insert( std::pair<std::string, hVal*>("SP",ef.new_hVal(hVal::FNCC,(void*)split_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("JN",ef.new_hVal(hVal::FNCC,(void*)join_f,0)));
    ...
    ec.f->insert( std::pair<std::string, hVal*>("\\+",ef.new_hVal(hVal::FNCC,(void*)swapPlus_f,0)));
    ...
    //-- переменные run-ов
    ec.f->insert( std::pair<std::string, hVal*>("v",ef.new_hVal(hVal::FNCC,(void*)v0_f,0)));
    ...
    ec.f->insert( std::pair<std::string, hVal*>("v30",ef.new_hVal(hVal::FNCC,(void*)v30_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v31",ef.new_hVal(hVal::FNCC,(void*)v31_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v32",ef.new_hVal(hVal::FNCC,(void*)v32_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v33",ef.new_hVal(hVal::FNCC,(void*)v33_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v34",ef.new_hVal(hVal::FNCC,(void*)v34_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v35",ef.new_hVal(hVal::FNCC,(void*)v35_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v36",ef.new_hVal(hVal::FNCC,(void*)v36_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v37",ef.new_hVal(hVal::FNCC,(void*)v37_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v38",ef.new_hVal(hVal::FNCC,(void*)v38_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v39",ef.new_hVal(hVal::FNCC,(void*)v39_f,0)));

    ec.f->insert( std::pair<std::string, hVal*>("v40",ef.new_hVal(hVal::FNCC,(void*)v40_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v41",ef.new_hVal(hVal::FNCC,(void*)v41_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v42",ef.new_hVal(hVal::FNCC,(void*)v42_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v43",ef.new_hVal(hVal::FNCC,(void*)v43_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v44",ef.new_hVal(hVal::FNCC,(void*)v44_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v45",ef.new_hVal(hVal::FNCC,(void*)v45_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v46",ef.new_hVal(hVal::FNCC,(void*)v46_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v47",ef.new_hVal(hVal::FNCC,(void*)v47_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v48",ef.new_hVal(hVal::FNCC,(void*)v48_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v49",ef.new_hVal(hVal::FNCC,(void*)v49_f,0)));

    ec.f->insert( std::pair<std::string, hVal*>("v50",ef.new_hVal(hVal::FNCC,(void*)v50_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v51",ef.new_hVal(hVal::FNCC,(void*)v51_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v52",ef.new_hVal(hVal::FNCC,(void*)v52_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v53",ef.new_hVal(hVal::FNCC,(void*)v53_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v54",ef.new_hVal(hVal::FNCC,(void*)v54_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v55",ef.new_hVal(hVal::FNCC,(void*)v55_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v56",ef.new_hVal(hVal::FNCC,(void*)v56_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v57",ef.new_hVal(hVal::FNCC,(void*)v57_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v58",ef.new_hVal(hVal::FNCC,(void*)v58_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("v59",ef.new_hVal(hVal::FNCC,(void*)v59_f,0)));
    ...
    ec.f->insert( std::pair<std::string, hVal*>(".I",ef.new_hVal(hVal::FNCC,(void*)sqI_f,0)));
...
//    ec.f->insert( std::pair<std::string, hVal*>("T+",ef.new_hVal(hVal::FNCC,(void*)utP_f,0)));// unix time +
    ...
    ec.f->insert( std::pair<std::string, hVal*>("TID",ef.new_hVal(hVal::FNCC,(void*)tid_f,0)));//
...
//    ec.f->insert( std::pair<std::string, hVal*>("UDP",ef.new_hVal(hVal::FNCC,(void*)udp_f,0)));
    ...
    // -- UU ???
//    ec.f->insert( std::pair<std::string, hVal*>("UU",ef.new_hVal(hVal::FNCC,(void*)uuid_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("UI",ef.new_hVal(hVal::FNCC,(void*)gtid_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("TMI",ef.new_hVal(hVal::FNCC,(void*)gtmi_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("H2D",ef.new_hVal(hVal::FNCC,(void*)SH2UL_f,0)));

    ec.f->insert( std::pair<std::string, hVal*>("S2U",ef.new_hVal(hVal::FNCC,(void*)toU_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("S2L",ef.new_hVal(hVal::FNCC,(void*)toL_f,0)));

    //-- URL DECODE-ENCODE
    ec.f->insert( std::pair<std::string, hVal*>("UD",ef.new_hVal(hVal::FNCC,(void*)urlD_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("UE",ef.new_hVal(hVal::FNCC,(void*)urlE_f,0)));

    // -- RegExp PCRE
    ec.f->insert( std::pair<std::string, hVal*>("RE",ef.new_hVal(hVal::FNCC,(void*)re_f,0)));
    ec.f->insert( std::pair<std::string, hVal*>("REQ",ef.new_hVal(hVal::FNCC,(void*)reQ_f,0)));
